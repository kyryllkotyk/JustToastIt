...
public:
	...
	// For creating with an ID
	Task(const std::string& assignedName, short diff,
		int time, const std::string& due,
		const std::string& dueDateFormat, const std::chrono::time_zone* zone,
		bool debugStatus/* = false*/, int ID);
...
private:
	...
	static TIME_POINT parseDateTime(
		const std::string& input, size_t expectedSize, size_t yearPos,
		size_t monthPos, size_t dayPos, size_t hourPos, size_t minutePos, 
		bool american, size_t yearSize = 4);
